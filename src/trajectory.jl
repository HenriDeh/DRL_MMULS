...
mutable struct PPOTrajectory{A <: AbstractArray}
    traces::NamedTuple{(:state, :action, :reward, :next_state, :action_log_prob, :advantage), NTuple{6, A}}
    idx::Dict{Symbol, Int}
    N::Int
end
...
function PPOTrajectory(N, state_size, action_size; device = cpu)
    ...
    PPOTrajectory(
        ...
        Dict(:state => 1, :action => 1, :reward => 1, :next_state => 1, :action_log_prob => 1, :advantage => 1),
        N
    )
end
...
function Base.rand(t::PPOTrajectory, n::Int)
    idx = rand(1:t.N, n)
    t.traces.state[:,idx], t.traces.action[:,idx], t.traces.action_log_prob[:,idx], t.traces.advantage[:,idx]
end
