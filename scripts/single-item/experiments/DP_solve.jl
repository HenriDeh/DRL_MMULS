...
function solve_adi_simple()
    ...
    CSV.write("data/single-item/scarf_testbed_adi_simple.csv", DataFrame(lostsales = Bool[], first_var = Float64[], var_discount = Float64[], forecast_id = Int[], simple_cost = Float64[], simple_MC_std = Float64[], solve_time_simple_s = Float64[]))
    leadtime = 2 
    setup = 320 
    CV = 0.2 
    forecast_horizon = 32
...
function solve_simple_lostsales()
    println("Solving SDI with simple (s,S)")
